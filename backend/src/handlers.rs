use crate::models::{Status, CreateTodo, EditTodo};
use crate::db;
...
pub async fn edit_todo_by_id(db_pool: web::Data<Pool>, path: web::Path<(i32,)>, json: web::Json<EditTodo>) -> impl Responder{
    let client: Client = 
        db_pool.get().await.expect("Error geting todo's from DB");
    let result = db::edit_todo_by_id(&client, path.0,  json.title.clone(), json.procent.clone(), json.deadline.clone()).await;
